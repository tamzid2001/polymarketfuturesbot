...
import kalshi_ml_market_data as market_data
...
@lru_cache(maxsize=1)
def legacy_execution_support() -> Any:
    """Load the retired standalone execution helper only when that CLI is used.

    The continuously deployed ML-side average-down runner never calls this
    function, so its inference path cannot import or use the legacy forecast
    implementation.
    """
    import kalshibtc15minupordown as legacy
    return legacy
...
def next_open_timestamp(ticker: str) -> pd.Timestamp:
    parsed = market_data.parse_ticker(ticker)
...
def executable_position_price(raw_market: Any, side: str) -> float | None:
    ...
    for field in fields:
        price = market_data.to_dollars(getattr(raw_market, field, None))
        if price is not None and 0.01 <= float(price) <= 0.99:
            return float(price)
    if side == "no":
        yes_price = market_data.to_dollars(getattr(raw_market, "last_price_dollars", None))
        if yes_price is None:
            yes_price = market_data.to_dollars(getattr(raw_market, "last_price", None))
...
def order_fields(side: str, position_price: float) -> tuple[Any, str]:
    legacy = legacy_execution_support()
    if side == "yes":
        return legacy.BookSide.BID, f"{position_price:.4f}"
    return legacy.BookSide.ASK, f"{1.0 - position_price:.4f}"
...
async def reconcile_pending_signals(rest: Any, state_path: Path) -> int:
...
def record_signal(
    ...
    *,
    ...
    signals[ticker] = {
        ...
        "decision": decision,
        "reason": reason,
        "dry_run": legacy_execution_support().DRY_RUN,
    }


async def wait_for_preopen() -> tuple[str, str]:
    while True:
        current_ticker, next_ticker = market_data.current_and_next_tickers()
        seconds_to_open = market_data.seconds_until_ticker_settle(current_ticker)
    ...
    *,
    ...
    try:
        candles = await asyncio.wait_for(
            loop.run_in_executor(None, market_data.fetch_btc_1m), timeout=PREOPEN_DATA_TIMEOUT_S,
        )
    except TimeoutError:
        LOG.warning(
        ...
        return None
    valid, reason = market_data.validate_data(candles)
    if not valid:
...
async def resolve_target_market(rest: Any, target_ticker: str) -> dict[str, Any] | None:
    legacy = legacy_execution_support()
    while True:
        market = await legacy.resolve_active_market(rest)
        if market and market.get("ticker") == target_ticker and market.get("target") is not None:
            return market
        since_open = legacy.seconds_since_ticker_open(target_ticker)
        if since_open is None or since_open > legacy.OPEN_TRADE_GRACE_S:
            return None
        await asyncio.sleep(1)
...
async def score_and_maybe_submit(
    ...
    current_ticker, _ = market_data.current_and_next_tickers()
    seconds_to_open = market_data.seconds_until_ticker_settle(current_ticker)
    ...
    if market is None or not legacy.is_within_open_trade_grace(
        legacy.seconds_since_ticker_open(target_ticker)
    ):
    ...
    response, filled = await legacy._submit(
        rest,
...
async def run(args: argparse.Namespace) -> None:
    legacy = legacy_execution_support()
    if args.submit and not legacy.DRY_RUN and not args.allow_live:
        raise SystemExit("Refusing a real order: add --allow-live with --submit.")
    rest = legacy.KalshiREST()
    try:
