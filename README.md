...
For an isolated non-trading monitor, use **Actions → “Kalshi BTC Weighted Exit-Protection Paper Reports”**. It runs normal and inverse studies for both ML and Prophet with `DRY_RUN=true`, separate state/history files, and no primary ladder. Each study freezes its model side before open, subscribes to the selected-side top-of-book, and records only fresh depth-supported 1×40¢/2×30¢/3×20¢/4×10¢ paper fills and VWAP. It runs two independent exit comparisons: the existing full-depth 10¢ trailing retracement and a regular **5¢ fixed stop below VWAP**. A fixed stop closes only when a fresh executable bid has displayed depth for the entire paper position and is at or below `VWAP − $0.05`; it uses the observed bid, never an assumed stop fill. Its default 5h45m run queues the next paper-only handoff, so the monitor continues 24/7. Material changes are checkpointed to `paper_reports/` on `main` while the Action is still running (at most about 10 seconds after an event) and uploaded as final artifacts. It never touches the live trader’s state, orders, or concurrency group.
...
The ML and Prophet Actions also run four independent weighted paper studies for each exit rule: normal ML, inverse ML, normal Prophet, and inverse Prophet. Each locks its side before the market opens: normal uses the prediction and inverse uses its exact opposite; later quotes cannot switch either side. Each ladder buys **1 contract at 40¢, 2 at 30¢, 3 at 20¢, and 4 at 10¢** (10 maximum). Its held VWAPs are 40¢ for 1 contract, 33.33¢ for 3, 26.67¢ for 6, and 20¢ for 10. The 10¢ trailing comparison exits only after a full-position executable bid retraces from a prior high; the separate 5¢ fixed-stop comparison exits only at a fresh full-depth bid at or below VWAP minus 5¢. Both close at the **observed bid**, never an assumed stop price. These are separate paper-only ledgers: they cannot submit, close, cancel, reserve, or change a live order. Every compact report includes an ordered P&L/equity time series, entry VWAP and rung fills, exit evidence, current streak, and longest winning/losing streak.
