...
The bounded 60-second opening-book sample remains available for detailed timing calibration, but it is no longer the horizon for the ≥53¢ research question. When the immutable initial selected-side ask is below 53¢, a compact analytics-only tracker watches every fresh selected-side executable ask until market close. On the first ask at or above 53¢ it persists the exact cent price, exchange/worker timestamps, seconds after open and signal, whether the occurrence was after 60 seconds, displayed ask depth, and whether the configured quantity was fully supported by that depth. It then continues retaining post-entry minimum/maximum asks and finalizes the hypothetical no-stop settlement W/L and gross per-share P&L. It never submits an auxiliary live or shadow order, never changes the primary GTC entry, and never touches recovery or permanent-base state. Legacy records are marked as partial coverage rather than being silently counted as complete no-hits.
