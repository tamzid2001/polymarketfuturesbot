...
The normal live mode remains `normal_ml_ladder`. The explicitly selected `inverse_ml_weighted_hold_gate` live mode instead locks the **opposite** frozen ML side and posts `1×40¢`, `2×30¢`, `3×20¢`, `4×10¢` GTC entries (10 contracts / $2.00 maximum principal). It cancels resting entry limits before any exit, verifies the exchange position, and submits only a reduce-only IOC exit backed by a fresh complete book with displayed depth for the full remaining position. Its hard stop is the selected-side **bid at 5¢**, independent of average entry. Its 10¢ trailing exit arms only after the selected-side executable bid reaches the **current average filled entry + 60¢**; a later averaging fill rebases the full-depth trailing high without disarming the strategy. The workflow field is blank by default so handoffs preserve the already persisted mode rather than silently changing it. This mode is high risk and its small paper sample is not a validation or profitability proof.
