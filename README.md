...
When a retrain replaces the active artifact, the registry retains the predecessor artifact. The first runner using the new model downloads both and scores them on each **same frozen pre-open feature vector**. Its state/report records both probabilities and sides, YES→NO/NO→YES changes, agreement rate, and—once settled—the directional result for each model. This is prospective transition monitoring, not a promotion test or executable-P&L claim.
