...
Use **Actions → “Kalshi BTC 15m ML-Side Average Down” → Run workflow**. It monitors only `KXBTC15M`. Its separate inverse ML shadow uses **1.00 paper contract per rung** for readable counterfactual P&L; that value is stored independently and cannot change, reserve capital for, or submit a primary live order. The ML source accepts only the schema `ml_only_raw_candles_settled_outcomes_v1`: BTC candle returns/volatility/range, strike distance, clock, and previously settled outcomes. It does not fit, call, or consume Prophet or another price forecast.

### Settlement-contrarian live source

`settlement_contrarian_weighted_hold_gate` replaces inverse-ML inference as an explicit live source. It is not an ML model and does not download, load, score, or compare any ML artifact. At **market open + 45 seconds**, it reads the latest finalized `KXBTC15M` market whose `settlement_ts` is no later than that frozen cutoff, predicts the **opposite** outcome, records that source ticker/result/timestamp in the trade ledger, and locks the chosen side. If no such settlement is available by the 90-second entry grace, it skips the market rather than entering late. A handoff resumes the already stored source signal; it never recalculates it using a later outcome.

The live position is exactly **1 contract at 40¢, 2 at 30¢, 3 at 20¢, and 4 at 10¢**—10 contracts and **$2.00 maximum principal** before fees. All four are market-close-expiring GTC limits on the one locked side. The selected-side executable bid has an **absolute 5¢ stop**. If the full-depth executable bid reaches the **current average filled entry + 60¢**, a 10¢ trailing retracement is armed and remains armed through settlement; a later averaging fill rebases the executable high for the larger position without disarming the exit. Before a live reduce-only IOC exit, every resting entry is canceled and the exchange position is rechecked. This is a real-money strategy when the workflow is live; its historical outcome-only 51.74% directional result does not establish fill-adjusted or fee-adjusted profitability.

For an isolated non-trading monitor, use **Actions → “Kalshi BTC Hold-Gate Trailing Paper Reports”**. It runs normal and inverse studies for both ML and Prophet with `DRY_RUN=true`, separate state/history files, and no primary ladder. Each study freezes its model side before open and continuously records actual fresh YES and NO bid/ask prices and displayed depth. It fills only the frozen selected side at 1×40¢/2×30¢/3×20¢/4×10¢, then compares 17 independent paper holds: trails that arm after +1¢ through +9¢, then +10¢/+20¢/…/+80¢ over that study’s actual average filled entry. Every variant has an **absolute 5¢ selected-side stop**—it does not subtract 5¢ from the average. Once a variant arms, its 10¢ trail stays armed through settlement unless it exits on a later retracement. It never assumes a stop fill. Its default 5h45m run queues the next paper-only handoff, so the monitor continues 24/7. Material changes are checkpointed in the dedicated `kalshi-ml-hold-gate-paper-reports` and `kalshi-prophet-hold-gate-paper-reports` branches, then uploaded as final artifacts; they never write to `main` while the live trader is running. It never touches the live trader’s state, orders, configuration, or concurrency group.
...
The normal live mode remains `normal_ml_ladder`. The legacy `inverse_ml_weighted_hold_gate` mode locks the **opposite** frozen ML side. The deployed `settlement_contrarian_weighted_hold_gate` mode instead locks the opposite of the latest causally available settled KXBTC15M outcome at open + 45 seconds and does not invoke ML at all. Both weighted modes post `1×40¢`, `2×30¢`, `3×20¢`, `4×10¢` GTC entries (10 contracts / $2.00 maximum principal). They cancel resting entry limits before any exit, verify the exchange position, and submit only a reduce-only IOC exit backed by a fresh complete book with displayed depth for the full remaining position. Their hard stop is the selected-side **bid at 5¢**, independent of average entry. Their 10¢ trailing exit arms only after the selected-side executable bid reaches the **current average filled entry + 60¢**; a later averaging fill rebases the full-depth trailing high without disarming the strategy. The workflow field is blank by default so handoffs preserve the already persisted mode rather than silently changing it. These modes are high risk; directional backtests and small paper samples are not validation or profitability proof.
