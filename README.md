...
For an isolated non-trading monitor, use **Actions → “Kalshi BTC Weighted Trailing Paper Reports”**. It runs normal and inverse studies for both ML and Prophet with `DRY_RUN=true`, separate state/history files, and no primary ladder. Each study freezes its model side before open, subscribes to the selected-side top-of-book, records only fresh depth-supported 1×40¢/2×30¢/3×20¢/4×10¢ paper fills, VWAP, target/high, and full-depth trailing exits. Material changes are checkpointed to `paper_reports/` on `main` while the Action is still running (at most about 10 seconds after an event) and uploaded as final artifacts. It never touches the live trader’s state, orders, or concurrency group.
