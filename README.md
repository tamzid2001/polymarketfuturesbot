...
| Delayed ≥53¢ analytics | **First qualifying ask through market close** | For a below-53¢ initial ask, records the first later fresh executable ask ≥53¢, including occurrences after the 60-second detailed opening capture; analytics only, never an order |
...
The bounded 60-second opening-book sample remains available for detailed timing calibration, but it is no longer the horizon for the ≥53¢ research question. When the immutable initial selected-side ask is below 53¢, a compact analytics-only tracker watches every fresh selected-side executable ask until market close. On the first ask at or above 53¢ it persists the exact cent price, exchange/worker timestamps, seconds after open and signal, whether the occurrence was after 60 seconds, displayed ask depth, and whether the configured quantity was fully supported by that depth. It then continues retaining post-entry minimum/maximum asks and finalizes the hypothetical no-stop settlement W/L and gross per-share P&L. From delayed-ladder contract v1 onward, that same trigger also activates an independent analytics-only 50/40/30/20/10¢ ladder with 1/2/4/8/16 shares. It uses the same durable public-trade-volume allocator as the first-60-second ladders: quotes establish touches, while post-trigger public volume establishes conservative simulated fills without claiming queue priority. The worker persists rung cursors, partial/full quantities, settlement P&L, and separate ≥53¢ through ≥59¢ delayed-entry cohort summaries. It never submits an auxiliary exchange order, never changes the primary GTC entry, and never touches recovery or permanent-base state. Legacy records are marked as partial or `legacy_without_ladder`; historical rung fills are never fabricated from later minimum-price summaries.
...
- Every audit JSONL record is appended, flushed, and `fsync`ed before the worker resumes order/position management. Its companion strategy state is atomically written and `fsync`ed immediately after every audit event; therefore a state transition, fill observation, stop event, funding failure, settlement, reconciliation result, and handoff is checkpointed while the worker is running—not merely at its end. Remote checkpoints are coalesced at `durable_checkpoint_interval_seconds` and force-update one parentless `runtime-state-kxbtc15m` snapshot with an exact lease. That branch starts from an empty Git tree and contains only the five explicitly supplied KXBTC15M durable files plus `.kxbtc15m-runtime-state.json`, whose owner/ref/schema are verified before restore. It cannot accumulate commit history or inherit another bot's tracked files, while `main` remains code-only and stable. The first upgraded worker may restore legacy `runtime-state` once if the new branch does not yet exist; its first material checkpoint completes the migration. A pending material event is retried by ordinary worker checkpoints once the interval expires.
