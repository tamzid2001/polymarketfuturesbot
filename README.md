...
For an isolated non-trading monitor, use **Actions → “Kalshi BTC Weighted Trailing Paper Reports”**. It runs normal and inverse studies for both ML and Prophet with `DRY_RUN=true`, separate state/history files, and no primary ladder. Each study freezes its model side before open, subscribes to the selected-side top-of-book, records only fresh depth-supported 1×40¢/2×30¢/3×20¢/4×10¢ paper fills, VWAP, target/high, and full-depth trailing exits. Its default 5h45m run queues the next paper-only handoff, so the monitor continues 24/7. Material changes are checkpointed to `paper_reports/` on `main` while the Action is still running (at most about 10 seconds after an event) and uploaded as final artifacts. It never touches the live trader’s state, orders, or concurrency group.
...
The ML and Prophet Actions also run four independent **weighted trailing paper studies**: normal ML, inverse ML, normal Prophet, and inverse Prophet. Each locks its side before the market opens: normal uses the prediction and inverse uses its exact opposite; later quotes cannot switch either side. Each ladder buys **1 contract at 40¢, 2 at 30¢, 3 at 20¢, and 4 at 10¢** (10 maximum). Its held VWAPs are 40¢ for 1 contract, 33.33¢ for 3, 26.67¢ for 6, and 20¢ for 10. It tracks gross opportunities at 1¢/2¢/3¢/5¢/10¢ and 20¢ through 60¢ above VWAP. After a full-position executable bid sets a high, a later fresh full-depth bid at least 10¢ below that high exits at the **observed bid**, never an assumed stop price. These are separate paper-only ledgers: they cannot submit, close, cancel, reserve, or change a live order. The ML Action persists a dedicated normal and inverse ledger/report pair (`kalshi_btc15m_weighted_trailing_{normal,inverse}_{ledger,report}.json`); the Prophet Action persists the corresponding `prophet_btc_weighted_trailing_{normal,inverse}_{history,report}.json` pairs. Every compact report includes an ordered P&L/equity time series, entry VWAP and rung fills, target/high/stop evidence, current streak, and longest winning/losing streak.
