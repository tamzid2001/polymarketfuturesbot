...
The input is parsed as UTC and timezone information is removed only after conversion, preserving UTC clock time for Prophet. The analysis is explicitly limited to the most recent `{config.max_trades}` completed trades, but selecting that lookback never changes any balance value. For original Kalshi positions, ticker timestamps are parsed with the documented `KXBTC15M` regex and realized `Total return ($)` forms the P/L series only after an explicit verified balance before the first source trade is supplied. Confirmed duplicate economic records only are removed; unresolved duplicate timestamps stop the run.

The always-on **shadow balance** is the original absolute balance curve and includes every realized opportunity P/L, including results during inactive periods. The filtered curve is anchored to the same absolute balance immediately before the first retained trade and includes only opportunities whose state was already on before that opportunity. At trade index *t*, the model is trained on shadow-balance rows ending at *t-1*, forecasts the exact timestamp of *t*, then the outcome is revealed. The P10/P90 comparison changes state only for *t+1*. This sequencing prevents the trade generating a signal from being counted in the new regime.
